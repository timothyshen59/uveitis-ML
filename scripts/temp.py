...
Examples:
  ...
  python train.py --epochs 10 --model_path checkpoints/best.pt

CSV split paths are configured in config.py under DATA, e.g.:
...
from models.VitB import ViTBaseModel, load_model, encode_ordinal, ordinal_to_class
...
def load_split(csv_path, label_cols):
    """Read a CSV split and drop rows with missing labels."""
...
def make_wandb_config(args, dcfg, mcfg, ocfg):
    """Shared config dict passed to wandb."""
    return {
        "epochs":       args.epochs,
...
def main():
    ...
    train_loader = make_loader(FundusDataset(train_df, dcfg["img_dir"]), range(len(train_df)), dcfg["batch_size"], dcfg["workers"], shuffle=True)
    val_loader   = make_loader(FundusDataset(val_df,   dcfg["img_dir"]), range(len(val_df)),   dcfg["batch_size"], dcfg["workers"], shuffle=False)
    test_loader  = make_loader(FundusDataset(test_df,  dcfg["img_dir"]), range(len(test_df)),  dcfg["batch_size"], dcfg["workers"], shuffle=False)
